  ...
  class ShishiMechanism {
    ...
    step(dt, count, waterMoment) {
      ...
      while (remaining > 1e-9) {
        ...
        const dryMoment = 1000 * Math.cos(this.angle);
        const inertia = 900 + Math.max(0, count) * 1.2;
        const acceleration = (waterMoment - dryMoment) / inertia - this.velocity * .5;
        this.velocity = Math.max(-.9, Math.min(.9, this.velocity + acceleration * h));
        this.angle += this.velocity * h;
        if (this.angle > .20) this.tipped = true;
