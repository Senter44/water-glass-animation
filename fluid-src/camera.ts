...
export class Camera {
    ...
    constructor (canvas: HTMLCanvasElement) {
        ...
        this.canvas.addEventListener("pointerdown", (event: PointerEvent) => {
            this.isDragging = (document.getElementById('interaction') as HTMLSelectElement).value === 'rotate';
        ...
        this.canvas.addEventListener("pointermove", (event: PointerEvent) => {
            ...
            this.currentHoverX = event.clientX - bounds.left;
            this.currentHoverY = event.clientY - bounds.top;
            if (this.isDragging) {
        ...
        this.canvas.addEventListener("pointerleave", () => { this.prevHoverX = this.currentHoverX; this.prevHoverY = this.currentHoverY; });
        window.addEventListener("resize", () => { if (this.fov) this.updateProjection(); });
    }

    updateProjection() {
        const projection = mat4.perspective(this.fov, this.canvas.clientWidth / this.canvas.clientHeight, 0.1, 300);
